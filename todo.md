...
| **M2a（单聊选 Bot）** | 🟡 评估锁定，待开干 | 端到端 **5–6 人日 + 0.5 buffer** |
| M2b（多选组群 + @ 路由） | ⬜ 后置 | Bridge 约 4–6.5 / 产品 5–8；建群权限提前单独开 |

**当前目标**：只做 **M2a**。M2b、CursorAgent 真接通、钉钉实现不进本轮。
...
- [ ] 飞书单聊：列表 → 选 A 对话 → 切 B → 确认会话已清且打到 B
...
| # | 验收项 | 负责人 | 状态 |
| --- | --- | --- | --- |
| 1 | 切换 Bot 必须清会话，且旧 agent 不出队抢答 | 研发 + 审核 | ⬜ |
| 2 | 未知/失效 agentId fail-closed，不能静默打到默认 | 中继必须做死 + 审核 | ⬜ |
| 3 | 绑定持久化，重启不丢 | 研发 + 审核 | ⬜ |
| 4 | V1 回归绿：mention / ACL / ack / 同 chat 连发顺序 | 研发 + 审核 | ⬜ |
...
- [ ] William 确认开干 M2a
- [ ] 初始 Bot 名单就绪
- [ ] 分支与 PR 策略确认（bridge / 中继）
- [ ] 本文件保持为唯一执行清单；进度每日回填勾选
